...
class PendularDraw(Node): # Instancia do nó que sera utilizado para fazer a comunicação 
    def __init__(self):
        ...
        # e chama o callback find_pose quando uma nova mensagem for recebida
        self.pose_subscriber_ = self.create_subscription(Pose, 'turtle1/pose', self.find_pose, 10) 
    ...
    def find_pose(self, pose: Pose):
        ...
        if pose.x > 5.5 and self.previous_x_ <= 5.5:
            ...
            self.set_color(255, 0, 0, 3, 0) # Chamando o serviço SetPen para definir a cor da linha como vermelha
        elif pose.x <= 5.5 and self.previous_x_ > 5.5:
            self.previous_x_ = pose.x
            self.get_logger().info("Branco")
            self.set_color(255, 255, 255, 3, 0) # Chamando o serviço SetPen para definir a cor da linha como branca
    
    # Método auxiliar que chama o serviço SetPen para alterar as cores da linha
    def set_color(self, r, g, b, width, off):
        ...
        future.add_done_callback(partial(self.callback_set_color))
        
    # Callback que é chamada quando a chamada ao serviço SetPen é concluída
    def callback_set_color(self, future):
        try:
